...
parser.add_argument("--dim", type=int, default=256, help="hidden size")
...
parser.add_argument("--extend", type=float, default=0.0, help="highlight region extension")
...
# our new hyperparameters
parser.add_argument('--beta', type=float, default=0.1, help='for lp-predictor')
parser.add_argument('--kl_lambda', type=float, default=1.0, help='predictor kl loss')
parser.add_argument('--mask_rate', type=float, default=0.15, help='mlm mask rate')
parser.add_argument('--mask_lambda', type=float, default=0.0, help='mlm mask lambda')
parser.add_argument('--sgpa_layers', type=int, default=2, help='feature encoder layers')
...
if configs.mode.lower() == 'train':
    ...
    for epoch in range(configs.epochs):
        ...
        for data in tqdm(train_loader, total=num_train_batches, desc='Epoch %3d / %3d' % (epoch + 1, configs.epochs)):
            global_step += 1
            _, vfeats, vfeat_lens, sentences, word_ids, char_ids, s_labels, e_labels, h_labels, NER_labels, word_masks = data
            # prepare features
            vfeats, vfeat_lens = vfeats.to(device), vfeat_lens.to(device)
            word_ids, char_ids, word_masks = word_ids.to(device), char_ids.to(device), word_masks.to(device)
            ...
            # compute logits
            h_score, start_logits, end_logits, text_logits, dn_s_logits, dn_e_logits, label_embs = model(word_ids, char_ids, vfeats, video_mask, query_mask, s_labels, e_labels, h_labels, word_masks, sentences)
            # compute loss
            highlight_loss = model.compute_highlight_loss(h_score, h_labels, video_mask)
            # highlight_loss = model.compute_NER_highlight_loss(h_score, NER_labels, label_embs, video_mask)
            loc_loss = model.compute_loss(start_logits, end_logits, s_labels, e_labels)
            kl_loss = model.compute_kl_loss(start_logits, end_logits, dn_s_logits, dn_e_logits, video_mask)
            # mask_loss = model.compute_mask_loss(text_logits, word_ids, word_masks)
            # total_loss = loc_loss + configs.highlight_lambda * highlight_loss + configs.kl_lambda * kl_loss + configs.mask_lambda * mask_loss
            total_loss = loc_loss + configs.highlight_lambda * highlight_loss + configs.kl_lambda * kl_loss
