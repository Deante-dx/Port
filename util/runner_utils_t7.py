...
def eval_test(model, data_loader, device, mode='test', epoch=None, global_step=None):
    ious = []
    with torch.no_grad():
        for idx, (records, vfeats, vfeat_lens, sentences, word_ids, char_ids) in tqdm(
            ...
            _, start_logits, end_logits, *_ = model(word_ids, char_ids, vfeats, video_mask, query_mask, sentences=sentences)
